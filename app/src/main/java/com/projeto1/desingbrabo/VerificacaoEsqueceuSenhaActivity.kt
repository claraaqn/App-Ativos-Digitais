...
class VerificacaoEsqueceuSenhaActivity : AppCompatActivity() {
        ...
        call.enqueue(object : Callback<GenericResponse> {
            override fun onResponse(call: Call<GenericResponse>, response: Response<GenericResponse>) {
                if (response.isSuccessful) {
                    response.body()?.let {
                        if (it.success) {
                            Toast.makeText(this@VerificacaoEsqueceuSenhaActivity, "Código validado!", Toast.LENGTH_SHORT).show()
                            val intent = Intent(this@VerificacaoEsqueceuSenhaActivity, EsqueceuSenhaActivity::class.java)
                            intent.putExtra("email", email)
                            startActivity(intent)
                            finish()
                        } else {
                            Toast.makeText(this@VerificacaoEsqueceuSenhaActivity, it.message, Toast.LENGTH_SHORT).show()
                        }
                    }
                } else {
                    Log.e("ValidarCodigo", "Erro: ${response.code()}")
                    Toast.makeText(this@VerificacaoEsqueceuSenhaActivity, "Erro ao validar código.", Toast.LENGTH_SHORT).show()
                }
            }

            override fun onFailure(call: Call<GenericResponse>, t: Throwable) {
                Log.e("ValidarCodigo", "Falha: ${t.message}")
                Toast.makeText(this@VerificacaoEsqueceuSenhaActivity, "Erro de conexão.", Toast.LENGTH_SHORT).show()
            }
        })
    }
        ...
        apiService.enviar_email_redefinicao(forgotPasswordRequest).enqueue(object : Callback<GenericResponse> {
            override fun onResponse(call: Call<GenericResponse>, response: Response<GenericResponse>) {
                if (response.isSuccessful && response.body()?.success == true) {
                    Toast.makeText(this@VerificacaoEsqueceuSenhaActivity, "Email reenviado", Toast.LENGTH_SHORT).show()
                } else {
                    Toast.makeText(this@VerificacaoEsqueceuSenhaActivity, "Tente novamente mais tarde.", Toast.LENGTH_SHORT).show()
                }
            }

            override fun onFailure(call: Call<GenericResponse>, t: Throwable) {
                Toast.makeText(this@VerificacaoEsqueceuSenhaActivity, "Erro ao enviar e-mail: ${t.message}", Toast.LENGTH_SHORT).show()
            }
        })
    }
        ...
        for (i in editTexts.indices) {
            ...
            currentEditText.addTextChangedListener(object : TextWatcher {
                ...
                override fun onTextChanged(s: CharSequence?, start: Int, before: Int, count: Int) {
                    // Se o usuário digitou algo, vai para o próximo campo
                    if (!s.isNullOrEmpty() && nextEditText != null && count > 0) {
                        nextEditText.requestFocus()
                    }
                    // Se o usuário apagou tudo, volta para o campo anterior
